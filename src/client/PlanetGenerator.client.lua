...
local function GetHeight(Normal)
	...
	local cliff = CliffHeight(Normal, continent)
...
local function GetTriangleColor(Normal)
	if ContinentMask(Normal) > 0.5 then
		return LandColorId
	else
		return OceanColorId
	end
...
local function MaxRadius(Radius)
	return PlanetData.BaseHeight +
		PlanetData.DetailHeight +
		(Radius * 0.03) +
		PlanetData.MountainHeight
end
...
for _, Face in ipairs(Faces) do
	...
	for y = 0, FaceResolution do
		...
		for x = 0, FaceResolution do
			...
			Vertices[y][x] = Mesh:AddVertex(SpherePoint)
		end
	end
