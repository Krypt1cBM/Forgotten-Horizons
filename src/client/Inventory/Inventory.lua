...
function Inventory:NextTransferableIndex(itemId, start, final)
	for i = start or 1, final or Inventory.TotalSlots do
		...
		if slot and slot.ItemId == itemId and Items[slot.ItemId].MaxStack > slot.Amount then
			return i
		end
	end
	for i = start or 1, final or Inventory.TotalSlots do
...
function Inventory:MoveSlot(from, to)
	...
	if from < 1 or from > self.TotalSlots then
		return
	end
	if to < 1 or to > self.TotalSlots then
		return
	end
